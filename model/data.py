...
def load_data(train_path, test_path, rel_dict_path):
...
class MyDataset(DataSet):
    def __init__(self, config, dataset, rel_vocab, is_test):
        ...
        self.tokenizer = BertTokenizer.from_pretrained('./bert-chinese-wwm')
...
def get_pred_iterator(config, dataset, rel_vocab, is_test=True, collate_fn=my_collate_fn):
    dataset = MyDataset(config, dataset, rel_vocab, is_test)
    return TorchLoaderIter(dataset=dataset,
                           collate_fn=collate_fn,
                           batch_size=config.batch_size if not is_test else 1,
